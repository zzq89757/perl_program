...
sub usage {
        print STDERR << "USAGE";
Description     Count_no_cut By zzq
...
my @insert;
my %hash;
`mkdir -m 777 count_out_v1.2`;
open COUNT,">count_out_v1.2/$countFile";
...
while(<BAM>){
    ...
    next if($getFirstPairFlag || !$getReadPairedFlag  || $getDupFlag || $getReadUnmappedFlag  || $getMateUnmappedFlag || $insertSize == 0);
...
$count_uniq = $clean_reads - $count_nu;
