...
my $r1UnexpctedInsertSize;
my $r1DifferentContig;
my $r1WithSATag;
my $r1UnexpectedOrientation;
my $r2UnexpctedInsertSize;
...
while(<BAM>){
	...
	if(!$getMateUnmappedFlag){
        ...
        ++ $r2ChimerasDenominator if(!$getFirstPairFlag);        
            if($line[6]!~/=/ || abs $insertSize >100000 || $line[11]=~/SA:Z:/ || !$orientation){
                if($getFirstPairFlag){
                    ++ $r1Chimeras;
                    ++ $r1UnexpctedInsertSize if(abs $insertSize >100000);
                    ++ $r1UnexpectedOrientation if(!$orientation);
                    ++ $r1DifferentContig if($line[6]!~/=/);
                    ++ $r1WithSATag if($line[11] =~/SA:Z:/);
                }else{           
                    ++ $r2Chimeras;
                    ++ $r2UnexpctedInsertSize if(abs $insertSize >100000);
                    ++ $r2UnexpectedOrientation if(!$orientation);
...
print "ReadCount\\Type\tUnexpctedInsertSize\tDifferentContig\tWithSATag\tUnexpectedOrientation\n";
print "FirstOfPair\t$r1UnexpctedInsertSize\t$r1DifferentContig\t$r1WithSATag\t$r1UnexpectedOrientation\n";
print "SecondOfPair\t$r2UnexpctedInsertSize\t$r2DifferentContig\t$r2WithSATag\t$r2UnexpectedOrientation\n";